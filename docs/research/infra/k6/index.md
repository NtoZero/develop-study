---
title: "k6 심층 조사 지도"
description: "k6 v2.0.0의 실행 의미론, 부하 모델, 측정 통계, 용량 산정과 병목 진단을 연결하는 조사 문서"
domain: "infra"
topic: "k6"
...
이 조사 묶음은 `k6 사용법`이 아니라 **성능 테스트가 어떤 모델을 세우고, k6가 그 모델을 어떻게 실행하며, 결과를 어디까지 믿을 수 있는지**를 설명한다. 단순 API 목록 대신 실행 의미론, 정량 모델, 구현·버전 경계, 실패 진단을 서로 연결했다.

## 조사 경계

- 기준 버전: Grafana k6 OSS `v2.0.0` (2026-05-11 공개된 최신 정식 릴리스)
...
| --- | --- | --- | --- | --- | --- | --- |
| 01 시스템 모델 | VU·iteration·request 분리 | scenario→VU→iteration | 처리량·동시성 관계 | client-side 측정 경계 | 잘못된 단위 해석 | 테스트 질문 정의 |
...
| 09 실습·자동화 | 재현 계약 | 로컬→CI 흐름 | pass/fail 기준 | v2.0.0 이미지 | 의도적 실패 | 회귀 운영 |

## 권장 읽기 순서

1. [성능 테스트의 시스템 모델](./01-system-model.md)
...
- [k6 v2.0.0 공식 릴리스](https://github.com/grafana/k6/releases/tag/v2.0.0)
